(ns twist_bioscience.main-test
  "Contract + behavioral test for the twist_bioscience-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "twist_bioscience-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'twist_bioscience.main-test)]

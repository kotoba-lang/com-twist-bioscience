(ns twist_bioscience.main
  "Kotodama WASM entrypoint for the twist_bioscience clean-room actor (L4) — Clojure port.
  ...
  facts (`twist_bioscience.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "twist_bioscience")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"twist_bioscience.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "twist_bioscience-compat" :tier tier :entities entities} 200])
...
(defn start! [] :twist_bioscience-compat/ready)
